...
import {Header, MainText, Title, Column, Row, StylePrint, Indent, BoldItalic} from '../styleComponents/styleComponents';

class StatementToPrint extends Component {
    render(){
        const {buyer, name, registrationNumber, address, nameDoc, serieNumber, registerNumber, issuedBy, issuedOn, nameNotary, dateStatement, nameCase, nameBuyer, gender} = this.props;
        const WifeOrHusband = gender !== 2 ? [' яка зареєстрована ','моїм чоловіком','мого чоловіка', 'яка підписала ', 'її']: [' який зареєстрований ','моєю дружиною', 'моєї дружини ', 'який підписав ', 'його'];
        const genderParty = gender === 2 ? ' яка зареєстрована ': ' який зареєстрований ';

        return (
            <StylePrint>
                <Header>
                    <h5>НОТАРІАЛЬНИМ ОРГАНАМ</h5>
                    <p><b>{nameCase}</b>, реєстраційний номер облікової картки платника податків <b>{registrationNumber}, </b>{WifeOrHusband[0]} за адресою: {address}
                    </p>
                </Header>

                <Title>ЗАЯВА</Title>
                <MainText>
                    <Indent>Я, <b>{name}</b>, даю свою згоду на КУПІВЛЮ {WifeOrHusband[1]} <b>{nameBuyer[4]}</b>, реєстраційний номер облікової картки платника податків <b>{buyer.registrationNumberBuyer}</b>, {genderParty} за адресою: <b>{buyer.addressBuyer}</b> нерухомого майна, а саме квартири пiд номером 30 (тридцять), що знаходиться в будинку під номером 2-А (два «А») на вулиці Піддубного Івана в місті Києві.</Indent>
                    ...
                    <p>Договір купівлі-продажу, який буде укладатися за участю {WifeOrHusband[2]}<b>{nameBuyer[1]},</b> відповідає інтересам сім’ї, умови купівлі (у тому числі зазначену в договорі ціну) ми попередньо обговорили і вважаємо вигідними для нас, а укладення цього договору відповідає нашому спільному волевиявленню.</p>
                ...
                <MainText style={{margin:'5% 0 0 0'}}>
                    ...
                    <p>Особу <b>{ nameCase }</b>, {WifeOrHusband[3]} документ, встановлено, {WifeOrHusband[4]} дієздатність та факт реєстрації шлюбу перевірено.</p>
                </MainText>
